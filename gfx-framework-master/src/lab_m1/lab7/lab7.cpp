...
{
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(0.6f, 0.7f, 0.9f));
    }
...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    GLint mat_shininess = glGetUniformLocation(shader->program, "material_shininess");
    glUniform1i(mat_shininess, materialShininess);

    GLint mat_kd = glGetUniformLocation(shader->program, "material_kd");
    glUniform1f(mat_kd, materialKd);

    GLint mat_ks = glGetUniformLocation(shader->program, "material_ks");
    glUniform1f(mat_ks, materialKs);
